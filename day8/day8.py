...
my_input_path = os.path.join(os.path.realpath(os.path.join(
    os.getcwd(), os.path.dirname(__file__))), 'data.txt')


def read_from_csv(my_input_path: str):
...
# given
forest = read_from_csv(my_input_path)
...
# right
right_visible_reversed, right_viewing_distance_reversed = is_visible([row[::-1] for row in forest])
...
# bottom
bottom_visible_reversed_transposed, bottom_viewing_distance_reversed_transposed = is_visible([row[::-1] for row in transpose(forest)])
bottom_visible = transpose([row[::-1] for row in bottom_visible_reversed_transposed])
bottom_viewing_distance = transpose([row[::-1] for row in bottom_viewing_distance_reversed_transposed])

# part 1, 2
count, score = 0, [[1] * len(row) for row in forest]
for i, row in enumerate(forest):
	for j, _ in enumerate(row):
		count = count + 1 if left_visible[i][j] or right_visible[i][j] or top_visible[i][j] or bottom_visible[i][j] else count
		score[i][j] = left_viewing_distance[i][j] * right_viewing_distance[i][j] * top_viewing_distance[i][j] * bottom_viewing_distance[i][j] 
print(count)  # 1849
